...
# Filter rows - SPECIES
anopheles_df = mosquito_data %>% 
  filter(species %in% c("anopheles_atroparvus", "anopheles_maculipennis"))

# Sum all the species
anopheles_df <- anopheles_df %>%
  group_by(trap_name, start_date, end_date, latitude, longitude, AMT, trapping_effort) %>%
  summarise(species = "anopheles", females = sum(females))

# Add year, month and week columns
anopheles_df = anopheles_df %>% mutate(year=year(start_date))
anopheles_df$year <- as.factor(anopheles_df$year)
anopheles_df = anopheles_df %>% mutate(month=month(start_date))
anopheles_df$month <- as.factor(anopheles_df$month)
anopheles_df = anopheles_df %>% mutate(week=week(start_date))
anopheles_df$week <- as.factor(anopheles_df$week)

# Raw data plot
anopheles_plot <- anopheles_df %>%
...
# filename <- "Plots/Anopheles/raw_data_plot.jpg"
...
for(i in 1:nrow(anopheles_df)){ 
  x0 <- anopheles_df$start_date[i]
  x1 <- anopheles_df$end_date[i]
  ...
  anopheles_df$TM[i] <- TM
  anopheles_df$TX[i] <- TX
  anopheles_df$TN[i] <- TN
  anopheles_df$HRM[i] <- HRM
  anopheles_df$PPT[i] <- PPT
  anopheles_df$mwime[i] <- mwime
  anopheles_df$TM7[i] <- TM7
  anopheles_df$TM14[i] <- TM14
  anopheles_df$TM21[i] <- TM21
  anopheles_df$TX7[i] <- TX7
  anopheles_df$TX14[i] <- TX14
  anopheles_df$TX21[i] <- TX21
  anopheles_df$TN7[i] <- TN7
  anopheles_df$TN14[i] <- TN14
  anopheles_df$TN21[i] <- TN21
  anopheles_df$HRM7[i] <- HRM7
  anopheles_df$HRM14[i] <- HRM14
  anopheles_df$HRM21[i] <- HRM21
  anopheles_df$PPT7[i] <- PPT7
  anopheles_df$PPT14[i] <- PPT14
  anopheles_df$PPT21[i] <- PPT21
  anopheles_df$mwi7[i] <- mwi7
  anopheles_df$mwi14[i] <- mwi14
  anopheles_df$mwi21[i] <- mwi21
}

# Growing degree days
anopheles_df <-  bind_rows(mclapply(1:nrow(as.data.frame(anopheles_df)), function(i){
  # Verbose
  if (i %% 50 == 0) {print(i)}
...
anopheles_df <- merge(anopheles_df, 
                  (traps %>% 
                     dplyr::select(trap_name, trap_type, land_use)), 
                  by = "trap_name")

# Save dataframe
write_csv(anopheles_df, "Data/Mosquito_data/Anopheles/anopheles_df.csv")
writexl::write_xlsx(anopheles_df, "Data/Mosquito_data/Anopheles/anopheles_df.xlsx")
...
# Read excel
anopheles_df <- read_excel("Data/Mosquito_data/Anopheles/anopheles_df.xlsx")

# Convert variables
anopheles_df$trap_name <- as.factor(anopheles_df$trap_name)
anopheles_df$AMT <- as.factor(anopheles_df$AMT)
anopheles_df$year <- factor(anopheles_df$year)
anopheles_df$month <- factor(anopheles_df$month, ordered = TRUE, levels = as.character(1:12))
anopheles_df$week <- factor(anopheles_df$week, ordered = TRUE, levels = as.character(1:53))
anopheles_df$trap_type <- as.factor(anopheles_df$trap_type)
anopheles_df$land_use <- as.factor(anopheles_df$land_use)

# Boxplots ---------------------------------------------------------------------
# Land use boxplot
boxplot_landuse <- ggplot(anopheles_df, aes(land_use, females)) +
  geom_boxplot() +
  theme_bw() +
  xlab("Land use") +
...
# Trap_name boxplot
boxplot_trap_name <- ggplot(anopheles_df, aes(trap_name, females)) +
  geom_boxplot() +
  theme_bw() +
  xlab("Trap name") +
  ylab("Mosquito counts") +
...
# Trap_type boxplot
boxplot_trap_type <- ggplot(anopheles_df, aes(trap_type, females)) +
  geom_boxplot() +
  theme_bw() +
  xlab("Trap type") +
  ylab("Mosquito counts") +
...
# Save workspace
save.image("Models/Anopheles/glm_models/full_model.RData")

# Final model
anopheles_model <- glmer.nb(females ~  scale(l21gdd) + scale(HRM21) + scale(PPT7) +
                              (1|year:trap_name) +
                              (1|trap_type) +
                              (1|land_use),
                            control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)),
                            data = anopheles_df)
save(anopheles_model, file= "Models/Anopheles/glm_models/anopheles_model.RData")
# load("Models/Anopheles/glm_models/anopheles_model.RData")

summary(anopheles_model)
car::vif(anopheles_model)

# Model and residuals check ----------------------------------------------------
simulationOutput <- simulateResiduals(fittedModel = anopheles_model, plot = F)
...
testCategorical(simulationOutput, catPred = anopheles_df$land_use)
...
fittedModel <- glmmTMB(females ~ scale(l21gdd) + scale(HRM21) + scale(PPT7) +
                         (1|year:trap_name) +
                         (1|trap_type) +
                         (1|land_use),
                       ziformula = ~1,
...
# Performance
check_model(anopheles_model)

# Bayesian models with BRMS ----------------------------------------------------
# neg binomial model
anopheles_model_nb <- brm(
  bf(formula = females ~ scale(l21gdd) + scale(HRM21) + scale(PPT7) +
       (1|land_use)
  ),
  data = anopheles_df,
...
save(anopheles_model_nb, file= "Models/Anopheles/brms_models/anopheles_model_nb.RData")
load("Models/Anopheles/brms_models/anopheles_model_nb.RData")

summary(anopheles_model_nb)
density_plots <- plot(anopheles_model_nb)
marginal_effects_plots <- marginal_effects(anopheles_model_nb)
pp_check(anopheles_model_nb)
...
loo1 <- loo(anopheles_model_nb)
loo2 <- loo(anopheles_model_zinb)
...
# Save workspace
save.image("Prediction/Anopheles/prediction_prep.RData")

# Predictions ------------------------------------------------------------------
# Delete all variables.
rm(list = ls())

# Read excel (Anopheles data)
anopheles_df <- read_excel("Data/Mosquito_data/Anopheles/anopheles_df.xlsx")
...
load("Prediction/Anopheles/prediction_prep.RData")
...
# pred <- newdata %>%
#   dplyr::mutate(
#     pp = predict(anopheles_model, newdata = newdata, type = c("response"), allow.new.levels = TRUE))
# 
...
#   flush.console()
#   
#   pp <- apply(posterior_predict(anopheles_model_nb, 
#                                 newdata = data_chunk,
...
# write.csv(pred, "Prediction/Anopheles/pred.csv")

pred <- read.csv("Prediction/Anopheles/pred.csv")
...
# st_write(newdata_sf, "Prediction/Anopheles/anopheles_spatial_prediction.shp", append=FALSE) #save shapefile
...
# filename <- "Plots/Anopheles/anopheles_map_0.jpg"
...
agg_newdata$week <- as.factor(agg_newdata$week)

temporal_variability_plot <- ggplot(data=agg_newdata,
                                    aes(x = week, y = pp, color = year, group = year)) +
  geom_point(size=0.3) + 
  geom_line(size=0.5) + 
  scale_color_brewer(palette = "Set1", name="Year", 
                     limits = c("2015", "2016", "2017", "2018"))

# filename <- "Plots/Anopheles/anopheles_temporal_variability.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)

# 4-years + average plot
mean_counts <- agg_newdata %>%
  dplyr::select(week, pp) %>% 
  group_by(week) %>%
  summarise(pp = mean(pp))

agg_newdata2 <- rbind(agg_newdata, mean_counts %>% mutate(year = "mean"))

temporal_variability_plot_2 <- ggplot(data = agg_newdata2, aes(x = week, y = pp, color = year, group = year)) +
  geom_point(size = 0.3) + 
  geom_line(linewidth = ifelse(agg_newdata2$year == "mean", 1, 0.5)) + 
  scale_color_brewer(palette = "Set1", name = "Year", 
...
# filename <- "Plots/Anopheles/anopheles_temporal_variability_2.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
# filename <- "Plots/Anopheles/plot_2015.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
# filename <- "Plots/Anopheles/plot_2016.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
# filename <- "Plots/Anopheles/plot_2017.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
# filename <- "Plots/Anopheles/plot_2018.jpg"
# ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
# Abundance by UTM1x1 ----------------------------------------------------------
anopheles_UTM1x1 <- pred %>%
  dplyr::select(UTM1X1, trap_name, long, lat, pp) %>%
  group_by(UTM1X1) %>%
  summarise(mean_counts = mean(pp))

anopheles_UTM1x1$predicted_value_anopheles <-
  (anopheles_UTM1x1$mean_counts - min(anopheles_UTM1x1$mean_counts)) /
  (max(anopheles_UTM1x1$mean_counts) - min(anopheles_UTM1x1$mean_counts))

# write_csv(anopheles_UTM1x1, "Prediction/Anopheles/anopheles_UTM1x1.csv")
